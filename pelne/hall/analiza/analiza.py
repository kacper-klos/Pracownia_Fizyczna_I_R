...
ohm_measurments = np.array(
    [
        [-26.82, -20.08, -24.74, -19.73, -9.69, -4.70],  # I [mA]
...
const_current_measurments = np.array(
    [
        [53.3, 46.3, 38.8, 32.5, 18.5, 6.7, 63.8, 57.5],  # B+ [mT]
        ...
        [-8.148, -7.145, -6.148, -5.161, -3.256, -1.196, -9.565, -8.688],
    ]
)  # U M- P- [mV]
...
const_magnetic_field_measurments = np.array(
    [
        ...
        [-9.561, -7.741, -5.963, -4.192, -2.426, -10.287, -0.972],
    ]
)  # U [mV] M- P-
...
def HallVoltageConstCurrent(measurments):
    hall_voltage = (
        measurments[2] - measurments[3] - measurments[4] + measurments[5]
    ) / 4
    hall_voltage_err = np.max(
        np.array(
            [
                MultimeterVoltageError(measurments[2], HALL_RANGE),
                MultimeterVoltageError(measurments[3], HALL_RANGE),
                MultimeterVoltageError(measurments[4], HALL_RANGE),
                MultimeterVoltageError(measurments[5], HALL_RANGE),
            ]
        ),
        axis=0,
    )
...
def MagneticFielsTotal(measurments):
    ...
    err = np.max(
        np.array([MagneticError(measurments[0]), MagneticError(measurments[1])]), axis=0
    )
    return field, err
...
def Conductivity(voltag, current, voltage_err, current_err):
    ...
    cond_err = (
        ...
        * np.sqrt(
            (current_err / voltag) ** 2 + (current * voltage_err / (voltag**2)) ** 2
        )
    )
...
def TempMobility(
    voltage, hall_voltage, field, voltage_err, hall_voltage_err, field_err
):
    mobility = hall_voltage / (field * voltage) * SAMPLE_LENGTH / SAMPLE_WIDTH
    mobility_err = mobility * np.sqrt(
        (hall_voltage_err / hall_voltage) ** 2
        + (field_err / field) ** 2
        + (voltage_err / voltage) ** 2
    )
    return mobility, mobility_err


def TempConcentration(
    hall_voltage, field, current, hall_voltage_err, field_err, current_err
):
    concentration = current * field / (SAMPLE_HEIGHT * ELECTRON_CHARGE * hall_voltage)
    concentration_err = concentration * np.sqrt(
        (current_err / current) ** 2
        + (field_err / field) ** 2
        + (hall_voltage_err / hall_voltage) ** 2
    )
...
def PlotLineFit(
    x, y, x_err, y_err, params, x_label, y_label, data_label, fit_label, title
):
...
def PlotTemperatures(cooling, heating):
    plt.errorbar(
        cooling[0],
        cooling[1],
        xerr=TempError(cooling[0]),
        yerr=MultimeterVoltageError(cooling[1], HALL_RANGE),
        fmt="bv",
        label="Chłodzenie próbki",
    )
    plt.errorbar(
        heating[0],
        heating[1],
        xerr=TempError(heating[0]),
        yerr=MultimeterVoltageError(heating[1], HALL_RANGE),
        fmt="r^",
        label="Grzanie próbki",
    )
...
def PlotTemperaturesConductivity(cooling, heating):
    plt.errorbar(
        ...
        1 / cooling[1],
        xerr=TempError(cooling[0]),
        yerr=MultimeterVoltageError(cooling[1], HALL_RANGE) / (cooling[1] ** 2),
        fmt="bv",
        label="Chłodzenie próbki",
    )
    plt.errorbar(
        ...
        1 / heating[1],
        xerr=TempError(heating[0]),
        yerr=MultimeterVoltageError(heating[1], HALL_RANGE) / (heating[1] ** 2),
        fmt="r^",
        label="Grzanie próbki",
    )
    plt.xlabel(r"$T \ [^\circ C]$")
...
def PlotTemperaturesConductivityAdjusted(cooling, heating):
    conductivity_cooling, conductivity_cooling_err = Conductivity(
        cooling[2],
        temp_current_sample,
        SampleVoltageError(cooling[2]),
        CurrentError(temp_current_sample),
    )
    conductivity_heating, conductivity_heating_err = Conductivity(
        heating[2],
        temp_current_sample,
        SampleVoltageError(heating[2]),
        CurrentError(temp_current_sample),
    )
    plt.errorbar(
        1 / cooling[0],
        np.log(conductivity_cooling),
        xerr=TempError(cooling[0]) / (cooling[0] ** 2),
        yerr=conductivity_cooling_err / conductivity_cooling,
        fmt="bv",
        label="Chłodzenie próbki",
    )
    plt.errorbar(
        1 / heating[0],
        np.log(conductivity_heating),
        xerr=TempError(heating[0]) / (heating[0] ** 2),
        yerr=conductivity_heating_err / conductivity_heating,
        fmt="r^",
        label="Grzanie próbki",
    )
...
def PlotTemperaturesMobility(cooling, heating, field, field_err):
    mobility_cooling, mobility_cooling_err = TempMobility(
        cooling[2],
        cooling[1],
        field,
        SampleVoltageError(cooling[2]),
        MultimeterVoltageError(cooling[1], HALL_RANGE),
        field_err,
    )
    mobility_heating, mobility_heating_err = TempMobility(
        heating[2],
        heating[1],
        field,
        SampleVoltageError(heating[2]),
        MultimeterVoltageError(heating[1], HALL_RANGE),
        field_err,
    )
    plt.errorbar(
        cooling[0],
...
def PlotTemperaturesConcentration(
    cooling, heating, field, current, field_err, current_err
):
    concentartion_cooling, concentartion_cooling_err = TempConcentration(
        cooling[1],
        field,
        current,
        MultimeterVoltageError(cooling[1], HALL_RANGE),
    ...
    concentartion_heating, concentartion_heating_err = TempConcentration(
        heating[1],
        field,
        current,
        MultimeterVoltageError(heating[1], HALL_RANGE),
    ...
    plt.errorbar(
        cooling[0],
        concentartion_cooling,
        xerr=TempError(cooling[0]),
        yerr=concentartion_cooling_err,
        fmt="bs",
        label="Chłodzenie próbki",
    )
    plt.errorbar(
        heating[0],
        concentartion_heating,
        xerr=TempError(heating[0]),
        yerr=concentartion_heating_err,
        fmt="rs",
        label="Grzanie próbki",
    )
    plt.xlabel(r"$T \ [^{\circ}C]$")
...
def PlotTemperaturesVoltage(cooling, heating):
    plt.errorbar(
        ...
        cooling[2],
        xerr=TempError(cooling[0]),
        yerr=SampleVoltageError(cooling[2]),
        fmt="bv",
        label="Chłodzenie próbki",
    )
    plt.errorbar(
        ...
        heating[2],
        xerr=TempError(heating[0]),
        yerr=SampleVoltageError(heating[2]),
        fmt="r^",
        label="Grzanie próbki",
    )
    plt.xlabel(r"$T \ [^\circ C]$")
...
def PlotTemperaturesLog(cooling, heating):
    plt.errorbar(
        ...
        cooling[1],
        xerr=np.abs(
            np.exp(-1 / cooling[0]) * TempError(cooling[0]) / (cooling[0] ** 2)
        ),
        yerr=MultimeterVoltageError(cooling[1], HALL_RANGE),
        fmt="bv",
        label="Chłodzenie próbki",
    ...
    plt.errorbar(
        ...
        heating[1],
        xerr=np.abs(
            np.exp(-1 / heating[0]) * TempError(heating[0]) / (heating[0] ** 2)
        ),
        yerr=MultimeterVoltageError(heating[1], HALL_RANGE),
        fmt="r^",
        label="Grzanie próbki",
...
def PlotTemperaturesExp(cooling, heating, exp):
    adjusted_cooling_temp, adjusted_cooling_temp_err = TempMeasurmentsExp(
        cooling[0], exp
    )
    adjusted_heating_temp, adjusted_heating_temp_err = TempMeasurmentsExp(
        heating[0], exp
    )
    plt.errorbar(
        adjusted_cooling_temp,
        cooling[1],
        xerr=adjusted_cooling_temp_err,
        yerr=MultimeterVoltageError(cooling[1], HALL_RANGE),
        fmt="bv",
        label="Chłodzenie próbki",
    )
    plt.errorbar(
        adjusted_heating_temp,
        heating[1],
        xerr=adjusted_heating_temp_err,
        yerr=MultimeterVoltageError(heating[1], HALL_RANGE),
        fmt="r^",
        label="Grzanie próbki",
    )
...
const_current_hall_voltage, const_current_hall_voltage_err = HallVoltageConstCurrent(
    const_current_measurments
)
const_current_field, const_current_field_err = MagneticFielsTotal(
    const_current_measurments
)
...
const_current_input_err = HallVoltageXInputError(
    const_current_value,
    CurrentError(const_current_value),
    const_current_field,
    const_current_field_err,
)


const_field_hall_voltage, const_field_hall_voltage_err = HallVoltageConstCurrent(
    const_magnetic_field_measurments
)
...
const_field_input = HallVoltageXInput(
    const_field_current,
    Magnetic(const_magnetic_field_value_p, const_magnetic_field_value_m),
)
const_field_input_err = HallVoltageXInputError(
    const_field_current,
    const_field_current_err,
    Magnetic(const_magnetic_field_value_p, const_magnetic_field_value_m),
    np.max(
        np.array(
            [
                MagneticError(const_magnetic_field_value_p),
                MagneticError(const_magnetic_field_value_m),
            ]
        ),
        axis=0,
    ),
...
const_current_params, const_current_params_err = FitToLinearModel(
    const_current_input,
    const_current_hall_voltage,
    const_current_input_err,
    const_current_hall_voltage_err,
)
const_field_params, const_field_params_err = FitToLinearModel(
    const_field_input,
    const_field_hall_voltage,
    const_field_input_err,
    const_field_hall_voltage_err,
)
...
resistance_params, resistance_params_err = FitToLinearModel(
    ohm_measurments[0],
    ohm_measurments[1],
    SampleVoltageError(ohm_measurments[0]),
    CurrentError(ohm_measurments[1]),
)
conductivity = SAMPLE_LENGTH / (SAMPLE_HEIGHT * SAMPLE_WIDTH * resistance_params[0])
conductivity_err = (
    resistance_params_err[0]
    * SAMPLE_LENGTH
    / (SAMPLE_HEIGHT * SAMPLE_WIDTH * (resistance_params[0] ** 2))
)
...
#
final_hall_const = (
    const_current_hall_constant / (const_current_hall_constant_err**2)
    + const_field_hall_constant / (const_field_hall_constant_err**2)
) / (1 / (const_current_hall_constant_err**2) + 1 / (const_field_hall_constant_err**2))
final_hall_const_err = 1 / np.sqrt(
    1 / (const_current_hall_constant_err**2) + 1 / (const_field_hall_constant_err**2)
)
# print(f"finall hall constant: {final_hall_const}, error: {final_hall_const_err}")
mobility = final_hall_const * conductivity
mobility_err = np.sqrt(
    (final_hall_const * conductivity_err) ** 2
    + (final_hall_const_err * conductivity) ** 2
)
...
conductivity_cooling, conductivity_cooling_err = Conductivity(
    cooling_measurments[2],
    temp_current_sample,
    SampleVoltageError(cooling_measurments[2]),
    CurrentError(temp_current_sample),
)
conductivity_heating, conductivity_heating_err = Conductivity(
    heating_measurments[2],
    temp_current_sample,
    SampleVoltageError(heating_measurments[2]),
    CurrentError(temp_current_sample),
)
conductivity_cooling_adj = np.log(conductivity_cooling[start:])
conductivity_cooling_adj_err = (
    conductivity_cooling_err[start:] / conductivity_cooling[start:]
)
conductivity_heating_adj = np.log(conductivity_heating[start:])
conductivity_heating_adj_err = (
    conductivity_heating_err[start:] / conductivity_heating[start:]
)
...
cooling_fit, cooling_fit_err = FitToLinearModel(
    temp_cooling_adj,
    conductivity_cooling_adj,
    temp_cooling_adj_err,
    conductivity_cooling_adj_err,
)
heating_fit, heating_fit_err = FitToLinearModel(
    temp_heating_adj,
    conductivity_heating_adj,
    temp_heating_adj_err,
    conductivity_heating_adj_err,
)
...
coefficient = (
    cooling_fit[0] / (cooling_fit_err[0] ** 2)
    + heating_fit[0] / (heating_fit_err[0] ** 2)
) / (1 / (cooling_fit_err[0] ** 2) + 1 / (heating_fit_err[0] ** 2))
coefficient_err = 1 / np.sqrt(
    1 / (cooling_fit_err[0] ** 2) + 1 / (heating_fit_err[0] ** 2)
)
